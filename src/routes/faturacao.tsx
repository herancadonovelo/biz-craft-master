...
export const Route = createFileRoute("/faturacao")({
  ...
  component: () => {
    const { faturas, clientes, encomendas, add, remove, update } = useStore();
    const [form, setForm] = useState({ numero: "", clienteId: "", encomendaId: "", valor: 0, iva: 23, estado: "rascunho" as const, data: new Date().toISOString().slice(0, 10) });
    return (
      <div className="space-y-6">
        <PageHeader title="Faturação" description="Emissão e acompanhamento de faturas." />
        <Card><CardContent className="grid gap-3 p-4 md:grid-cols-6">
          ...
          <div className="md:col-span-6"><Button onClick={() => { if (!form.numero) return toast.error("Número obrigatório"); add("faturas", form); setForm({ numero: "", clienteId: "", encomendaId: "", valor: 0, iva: 23, estado: "rascunho", data: new Date().toISOString().slice(0, 10) }); toast.success("Fatura criada"); }}><Plus className="mr-1 h-4 w-4" />Criar fatura</Button></div>
        </CardContent></Card>
